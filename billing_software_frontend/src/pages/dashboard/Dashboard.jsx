...
import {
  ...
  BarChart2, Wallet, Clock, IndianRupee, Bell, ChevronDown,
...
useEffect(()=>{
  ...
  const TAB_DEFS = [
    { key: "sales",       label: "Monthly Sales",         icon: BarChart2 },
    { key: "lowstock",    label: "Low Stock Products",    icon: AlertTriangle },
    { key: "outstanding", label: "Outstanding Customers", icon: Wallet },
  ];
      ...
      {!selectedCompany ? (
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 20 }}>
            <StatCard label="Outstanding"      value={creditStats.total_outstanding}  icon={Wallet}        accent="red"    delay={0.18} prefix="₹" />
            <StatCard label="Overdue"          value={creditStats.overdue_amount}     icon={Clock}         accent="orange" delay={0.22} prefix="₹" />
            <StatCard label="Today Collection" value={creditStats.today_collection}   icon={IndianRupee}   accent="green"  delay={0.26} prefix="₹" />
          </div>

          {/* ── Chart / Tables Tabs ── */}
          <div style={{ background: "#fff", border: "1.5px solid #e0e7ff", borderRadius: 20, overflow: "hidden", marginBottom: 20, animation: "db-up .45s .32s ease both" }}>
            {/* Tab bar */}
            <div style={{ display: "flex", gap: 4, padding: "8px 10px 0 10px", borderBottom: "1px solid #eef1f5", background: "#fafbff" }}>
              {TAB_DEFS.map((t) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: active ? "0 -3px 10px rgba(99,102,241,.06)" : "none",
                      fontFamily: font,
                    }}
                  >
                    <t.icon size={15} color={active ? "#6366f1" : "#94a3b8"} />
                    {t.label}
                    {t.key === "lowstock" && lowStockProducts.length > 0 && (
                      <span style={{ background: "#fff1f2", color: "#be123c", borderRadius: 20, padding: "1px 8px", fontSize: 10.5, fontWeight: 700, border: "1px solid #fecdd3" }}>
            ...
            <div style={{ padding: "20px 24px" }}>
              ...
              {chartTab === "lowstock" && (
                <>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 800, color: "#1e1b4b" }}>Low Stock Products</div>
                      <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 2 }}>Items with 5 or fewer units remaining</div>
                    </div>
              ...
              {chartTab === "outstanding" && (
                <>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 800, color: "#1e1b4b" }}>Outstanding Customers</div>
                      <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 2 }}>Pending credit payments</div>
                    </div>
                    <div style={{ width: 34, height: 34, borderRadius: 10, background: "#eef2ff", display: "flex", alignItems: "center", justifyContent: "center" }}>
