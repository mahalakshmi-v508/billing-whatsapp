...
const COLORS = ["#10b981", "#6366f1", "#f59e0b", "#7c3aed", "#0891b2", "#f43f5e", "#eab308"];
  ...
  return (
    <div className="bg-slate-900/95 backdrop-blur-md text-white rounded-xl px-3.5 py-2.5 shadow-2xl border border-slate-700/50 text-xs">
      ...
      <div className="text-slate-400 mt-1">
        Profit: <span className="text-white font-semibold">{fmtINR(item.value)}</span>
      </div>
...
export default function PartyWiseProfitLossAnalytics({ rows = [], period = "", onClose }) {
  const totals = useMemo(() => {
    ...
    return { sale, profit, count, win, loss: count - win };
  }, [rows]);

  const topParties = useMemo(
    () =>
      [...rows]
        .sort((a, b) => Number(b.total_sale_amount || 0) - Number(a.total_sale_amount || 0))
        .slice(0, 8),
    [rows]
  );

  const profitDonut = useMemo(() => {
    const winners = rows
      .filter((r) => Number(r.profit || 0) >= 0)
      .sort((a, b) => Number(b.profit || 0) - Number(a.profit || 0));
    const TOP = 5;
    const top = winners.slice(0, TOP);
    const rest = winners.slice(TOP);
    const data = top.map((r, i) => ({
      name: r.party_name || "General",
    ...
    if (rest.length) {
      data.push({
        name: "Others",
        value: rest.reduce((a, r) => a + Number(r.profit || 0), 0),
        color: "#cbd5e1",
        isOther: true,
      });
    }
    const total = data.reduce((a, s) => a + s.value, 0) || 1;
    return data.filter((d) => d.value > 0).map((d) => ({ ...d, percent: ((d.value / total) * 100).toFixed(0) }));
  }, [rows]);

  const avgProfit = totals.count ? totals.profit / totals.count : 0;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          ...
          <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">
            Party Wise Profit & Loss Analytics
          </h1>
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-emerald-600">{period}</span> • {totals.count} part
        ...
        {typeof onClose === "function" && (
          <div className="flex items-center gap-2.5 flex-wrap">
            <button
              ...
              title="Return to Party Wise Profit & Loss Report"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Gross sales across all parties
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            {totals.win} profitable · {totals.loss} at loss
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Parties — Sales vs Profit</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Gross sales and resulting profit for the highest-selling parties
            </p>
              ...
              <div className="h-64 sm:h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={topParties} margin={{ top: 10, right: 10, left: -6, bottom: 0 }}>
                    ...
                    <Bar dataKey="total_sale_amount" name="Sales" fill="#6366f1" radius={[6, 6, 0, 0]} maxBarSize={34} />
                    <Bar dataKey="profit" name="Profit" fill="#10b981" radius={[6, 6, 0, 0]} maxBarSize={34} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">Profit by Party</h3>
              <span className="text-xs font-bold text-slate-400">{profitDonut.length} parties</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Share of profit contributed by each party</p>

            <div className="relative h-44 w-full flex items-center justify-center">
              {profitDonut.length === 0 ? (
                <div className="text-slate-400 text-xs">No profitable parties</div>
              ) : (
                <>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Party Wise Profit & Loss</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Sales, profit and margin for every party in the period
            </p>
          </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {[...rows]
                  .sort((a, b) => Number(b.profit || 0) - Number(a.profit || 0))
                  .map((r, i) => {
                    const margin = Number(r.total_sale_amount) > 0 ? (Number(r.profit) / Number(r.total_sale_amount)) * 100 : 0;
                    const share = totals.profit > 0 ? (Math.abs(Number(r.profit)) / Math.abs(totals.profit)) * 100 : 0;
                    const profitable = Number(r.profit) >= 0;
                    return (
                      <tr key={`${r.party_id || r.party_name}-${i}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                        ...
                        <td className="py-3 px-3 font-bold text-slate-800 max-w-[220px] truncate" title={r.party_name}>
                          {r.party_name}
                        </td>
                        <td className="py-3 px-3 text-right font-bold text-slate-600">{fmtINR(r.total_sale_amount)}</td>
                        <td className={`py-3 px-3 text-right font-extrabold ${profitable ? "text-emerald-600" : "text-rose-600"}`}>
                          {fmtINR(r.profit)}
                        </td>
                        <td className="py-3 px-3 text-right">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${profitable ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>
                            <TrendingDown className="w-3 h-3" />
                            {margin.toFixed(1)}%
                          </span>
                        </td>
                        <td className="py-3 pl-3 min-w-[130px]">
                          <div className="flex items-center gap-2">
                            <div className="w-full h-1.5 rounded-full bg-slate-100 overflow-hidden">
                              <div className={`h-1.5 rounded-full ${profitable ? "bg-emerald-500" : "bg-rose-400"}`} style={{ width: `${Math.min(100, share)}%` }} />
                            </div>
                          </div>
                        </td>
                      </tr>
