...
export default function PartyReportByItemAnalytics({ rows = [], period = "", onClose }) {
  const totals = useMemo(() => {
    let sale = 0,
      ...
      count = 0;
    rows.forEach((r) => {
      sale += Number(r.sale_amt || 0);
      purchase += Number(r.purchase_amt || 0);
      saleQty += Number(r.sale_qty || 0);
      purchaseQty += Number(r.purchase_qty || 0);
      count += 1;
    });
    return { sale, purchase, net: sale - purchase, saleQty, purchaseQty, count };
  }, [rows]);

  const topParties = useMemo(
    () =>
      [...rows]
        .sort((a, b) => Number(b.sale_amt || 0) - Number(a.sale_amt || 0))
        .slice(0, 8),
    [rows]
  );

  const saleDonut = useMemo(() => {
    const bySale = [...rows].sort((a, b) => Number(b.sale_amt || 0) - Number(a.sale_amt || 0));
    const TOP = 5;
    ...
    if (rest.length) {
      data.push({
        name: "Others",
        value: rest.reduce((a, r) => a + Number(r.sale_amt || 0), 0),
        color: "#cbd5e1",
        isOther: true,
      });
    }
  ...
  const maxSale = Math.max(...rows.map((r) => Number(r.sale_amt || 0)), 1);
    ...
    <div className="space-y-6 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-indigo-600 uppercase tracking-wider">
            <span>Party</span>
            <span>•</span>
            <span>Report By Item Analytics</span>
          ...
          <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">
            Party Report By Item Analytics
          </h1>
        ...
        {typeof onClose === "function" && (
          <div className="flex items-center gap-2.5 flex-wrap">
            <button
              ...
              title="Return to Party Report By Item"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            {totals.purchaseQty} units purchased
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Parties — Sale vs Purchase</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Itemised sale and purchase amounts for the top-selling parties
            </p>
              ...
              <div className="h-64 sm:h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={topParties} margin={{ top: 10, right: 10, left: -6, bottom: 0 }}>
                    ...
                    <Bar dataKey="sale_amt" name="Sale" fill="#6366f1" radius={[6, 6, 0, 0]} maxBarSize={34} />
                    <Bar dataKey="purchase_amt" name="Purchase" fill="#f59e0b" radius={[6, 6, 0, 0]} maxBarSize={34} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">Sale by Party</h3>
              <span className="text-xs font-bold text-slate-400">{saleDonut.length} parties</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Sales value shared by each party</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Party Item Movement</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Quantities and amounts of items sold and purchased per party
            </p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100">
                  ...
                  <th className="py-2.5 px-3 font-bold text-right">Sale Qty</th>
                  <th className="py-2.5 px-3 font-bold text-right">Sale Amt</th>
                  <th className="py-2.5 px-3 font-bold text-right">Purchase Qty</th>
                  <th className="py-2.5 px-3 font-bold text-right">Purchase Amt</th>
              ...
              <tbody>
                {[...rows]
                  .sort((a, b) => Number(b.sale_amt || 0) - Number(a.sale_amt || 0))
                  .map((r, i) => {
                    const net = Number(r.sale_amt || 0) - Number(r.purchase_amt || 0);
                    const share = (Number(r.sale_amt || 0) / maxSale) * 100;
                    return (
                      <tr key={`${r.party_id || r.name}-${i}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                        ...
                        <td className="py-3 px-3 text-right text-slate-500 font-semibold">{Number(r.sale_qty || 0)}</td>
                        <td className="py-3 px-3 text-right font-extrabold text-indigo-600">{fmtINR(r.sale_amt)}</td>
                        <td className="py-3 px-3 text-right text-slate-500 font-semibold">{Number(r.purchase_qty || 0)}</td>
                        <td className="py-3 px-3 text-right font-bold text-amber-600">{fmtINR(r.purchase_amt)}</td>
