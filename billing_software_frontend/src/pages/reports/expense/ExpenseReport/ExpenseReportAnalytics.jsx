...
export default function ExpenseReportAnalytics({ rows = [], period = "", onClose }) {
  const data = useMemo(
    () =>
      (rows || []).map((r) => ({
        date: r.expense_date || "",
        expNo: r.expense_no || r.id || "-",
        party: r.party_name || "-",
        category: r.category_name || "General",
        paymentType: r.payment_type || "Cash",
        amount: Number(r.total_amount || 0),
        paid: Number(r.paid_amount || 0),
        balance: Number(r.balance_amount || 0) > 0 ? Number(r.balance_amount || 0) : Math.max(0, Number(r.total_amount || 0) - Number(r.paid_amount || 0)),
      })),
    [rows]
  );
  ...
  const totals = useMemo(() => {
    ...
    data.forEach((r) => {
      ...
      paid += r.paid;
      balance += r.balance;
      if (r.party && r.party !== "-") parties.add(r.party);
    ...
    return {
      ...
      avg: data.length ? amount / data.length : 0,
      paidPct: amount ? (paid / amount) * 100 : 0,
    };
  }, [data]);
  ...
  const donut = useMemo(() => {
    ...
    const rest = catData.slice(TOP);
    const arr = top.map((c, i) => ({ name: c.name, value: c.value, color: COLORS[i % COLORS.length] }));
    if (rest.length) {
      arr.push({ name: "Others", value: rest.reduce((a, c) => a + c.value, 0), color: "#cbd5e1", isOther: true });
    }
    const total = arr.reduce((a, s) => a + s.value, 0) || 1;
    return arr.map((d) => ({ ...d, percent: ((d.value / total) * 100).toFixed(0) }));
  }, [catData]);

  const maxRow = Math.max(...data.map((r) => r.amount), 1);
    ...
    <div className="space-y-6 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          ...
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-indigo-600">{period}</span> • Expense transactions •{" "}
        ...
        {typeof onClose === "function" && (
          <button
            ...
            title="Return to Expense Report"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Gross expenditure in this period
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div>
            ...
            <div className="text-2xl sm:text-3xl font-black text-emerald-600 mt-2 tracking-tight">
              {totals.parties} <span className="text-sm font-bold text-emerald-400">/</span> {totals.categories}
            </div>
          </div>
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Unique parties and spending channels
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Total Settled</span>
          <span className="text-sm font-black text-emerald-600">{fmtINR(totals.paid)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Balance Due</span>
          <span className="text-sm font-black text-amber-600">{fmtINR(totals.balance)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Settlement Rate</span>
          <span className="text-sm font-black text-indigo-600">{totals.paidPct.toFixed(1)}%</span>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Categories by Expenditure</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Highest spend per expense category
            </p>
            ...
            {catData.length === 0 ? (
              ...
              <div className="h-64 sm:h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={catData.slice(0, 8)} margin={{ top: 10, right: 10, left: -6, bottom: 0 }}>
                    ...
                    <Bar dataKey="value" name="Expense" fill="#6366f1" radius={[6, 6, 0, 0]} maxBarSize={36} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">Expense by Category</h3>
              <span className="text-xs font-bold text-slate-400">{totals.categories} categories</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Where your money was spent</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Expense Vouchers by Amount</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Highest expense records in this period
            </p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100">
                  ...
                  <th className="py-2.5 px-3 font-bold">Exp No</th>
              ...
              <tbody>
                {[...data]
                  ...
                  .slice(0, 20)
                  .map((r, i) => {
                    const share = (Number(r.amount) / maxRow) * 100;
                    return (
                      <tr key={`${i}-${r.date}-${r.expNo}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                        <td className="py-3 pr-3">
                          <span className="w-5 h-5 rounded-md bg-indigo-100 text-indigo-700 font-bold text-[10px] flex items-center justify-center">
                            {i + 1}
                          </span>
                        </td>
                        <td className="py-3 px-3 font-semibold text-slate-600 whitespace-nowrap">{fmtDate(r.date)}</td>
                        <td className="py-3 px-3 font-bold text-slate-800 whitespace-nowrap">{r.expNo}</td>
                        <td className="py-3 px-3 font-semibold text-slate-700 max-w-[180px] truncate" title={r.party}>
                          {r.party}
                        </td>
                        <td className="py-3 px-3 font-semibold text-slate-600 max-w-[160px] truncate" title={r.category}>
                          {r.category}
                        </td>
                        <td className="py-3 px-3">
                          <span className="inline-flex px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 font-bold text-[10px]">
                            <Capitalized text={r.paymentType} />
                          </span>
                        </td>
                        <td className="py-3 px-3 text-right font-extrabold text-indigo-600">{fmtINR(r.amount)}</td>
