...
export default function ExpenseCategoryReportAnalytics({ rows = [], period = "", onClose }) {
  const data = useMemo(
    () =>
      (rows || [])
  ...
  const totals = useMemo(() => {
    ...
    const direct = data
      .filter((r) => String(r.type || "").toLowerCase().includes("direct"))
      .reduce((s, r) => s + r.amount, 0);
    const indirect = data
      .filter((r) => String(r.type || "").toLowerCase().includes("indirect"))
      .reduce((s, r) => s + r.amount, 0);
    return {
      amount,
      count: data.length,
  ...
  const donut = useMemo(() => {
    ...
    const arr = top.map((c, i) => ({ name: c.name, value: c.amount, color: COLORS[i % COLORS.length] }));
    if (rest.length) {
      arr.push({ name: "Others", value: rest.reduce((a, c) => a + c.amount, 0), color: "#cbd5e1", isOther: true });
    }
    ...
    <div className="space-y-6 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          ...
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-rose-600">{period}</span> • Direct &amp; indirect overheads •{" "}
        ...
        {typeof onClose === "function" && (
          <button
            ...
            title="Return to Expense Category Report"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Direct Expenses</span>
          <span className="text-sm font-black text-indigo-600">{fmtINR(totals.direct)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Indirect Expenses</span>
          <span className="text-sm font-black text-violet-600">{fmtINR(totals.indirect)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Top Category Share</span>
          <span className="text-sm font-black text-rose-500">
            {catData[0] ? `${((catData[0].amount / (totals.amount || 1)) * 100).toFixed(0)}%` : "-"}
          </span>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Highest category expenditure in this period
            </p>
              ...
              <div className="h-64 sm:h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={catData.slice(0, 8)} margin={{ top: 10, right: 10, left: -6, bottom: 0 }}>
                    ...
                    <Bar dataKey="amount" name="Expense" fill="#6366f1" radius={[6, 6, 0, 0]} maxBarSize={36} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs font-bold text-slate-400">{totals.count} categories</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Direct vs indirect spending mix</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Category-wise spend ranked by total amount
            </p>
          </div>
                  ...
                  return (
                    <tr key={`${i}-${r.name}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                      ...
                      <td className="py-3 px-3">
                        <span className="inline-flex px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 font-bold text-[10px]">
                          <Capitalized text={r.type} />
                        </span>
                      </td>
