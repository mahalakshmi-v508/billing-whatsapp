...
export default function ExpenseItemReportAnalytics({ rows = [], period = "", onClose }) {
  const data = useMemo(
    () =>
      (rows || []).map((r) => ({
        name: r.item_name || "General",
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      avgQty: data.length ? quantity / data.length : 0,
      maxUnit: Math.max(...data.map((r) => r.unitPrice), 0),
    };
  }, [data]);
  ...
  const itemData = useMemo(() => {
    const map = new Map();
    data.forEach((r) => {
      const cur = map.get(r.name) || { name: r.name, value: 0, quantity: 0 };
      cur.value += r.amount;
  ...
  const donut = useMemo(() => {
    const TOP = 5;
    const top = itemData.slice(0, TOP);
    const rest = itemData.slice(TOP);
    ...
    <div className="space-y-6 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          ...
          <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">
            Expense Item Analytics
          </h1>
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-rose-600">{period}</span> • Itemized procurement spend •{" "}
        ...
        {typeof onClose === "function" && (
          <button
            ...
            title="Return to Expense Item Report"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Avg Quantity / Line</span>
          <span className="text-sm font-black text-slate-800">{fmtQty(totals.avgQty)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Highest Unit Price</span>
          <span className="text-sm font-black text-blue-600">{fmtINR(totals.maxUnit)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Top Item Share</span>
          <span className="text-sm font-black text-rose-500">
            {itemData[0] ? `${((itemData[0].value / (totals.amount || 1)) * 100).toFixed(0)}%` : "-"}
          </span>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Expense Items</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Highest line amounts across expense vouchers
            </p>
            ...
            {itemData.length === 0 ? (
              ...
              <div className="h-64 sm:h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={itemData.slice(0, 8)} margin={{ top: 10, right: 10, left: -6, bottom: 0 }}>
                    ...
                    <Bar dataKey="value" name="Amount" fill="#f43f5e" radius={[6, 6, 0, 0]} maxBarSize={36} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">Expense by Item</h3>
              <span className="text-xs font-bold text-slate-400">{totals.count} items</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Where your procurement spend concentrated</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Expense Items by Amount</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Item-wise lines ranked by spend
            </p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100">
                  ...
                  <th className="py-2.5 px-3 font-bold">Expense Item</th>
              ...
              <tbody>
                {[...data]
                  ...
                  .slice(0, 20)
                  .map((r, i) => {
                    const share = (r.amount / maxRow) * 100;
                    return (
                      <tr key={`${i}-${r.name}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                        <td className="py-3 pr-3">
                          <span className="w-5 h-5 rounded-md bg-rose-100 text-rose-700 font-bold text-[10px] flex items-center justify-center">
                            {i + 1}
                          </span>
                        </td>
                        <td className="py-3 px-3 font-bold text-slate-800 max-w-[260px] truncate" title={r.name}>
                          {r.name}
                        </td>
                        <td className="py-3 px-3 text-right font-semibold text-slate-600 whitespace-nowrap">{fmtINR(r.unitPrice)}</td>
                        <td className="py-3 px-3 text-right">
                          <span className="inline-flex px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 font-bold text-[10px]">
                            {fmtQty(r.quantity)}
                          </span>
                        </td>
                        <td className="py-3 px-3 text-right font-extrabold text-rose-600">{fmtINR(r.amount)}</td>
                        <td className="py-3 pl-3 min-w-[120px]">
                          <div className="flex items-center gap-2">
                            <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden shrink-0">
                              <div className="bg-rose-500 h-1.5 rounded-full" style={{ width: `${Math.min(100, share)}%` }} />
                            </div>
