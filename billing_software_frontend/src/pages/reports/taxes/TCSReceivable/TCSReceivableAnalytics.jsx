...
export default function TCSReceivableAnalytics({ rows = [], period = "", onClose }) {
  const data = useMemo(
    () =>
      (rows || []).map((r) => ({
        name: r.party_name || "General",
        bill: r.bill_no || "-",
        date: r.date_of_collection || "",
        section: r.tax_section || "-",
        rate: Number(r.tcs_rate || 0),
        totalValue: Number(r.total_value || 0),
        paid: Number(r.amount_paid || 0),
        value: Number(r.total_tcs_paid || 0),
      })),
    [rows]
  );
  ...
  const totals = useMemo(() => {
    ...
    data.forEach((r) => {
      ...
      paid += r.paid;
      if (r.rate > topRate) topRate = r.rate;
      if (r.name) parties.add(r.name);
    });
    return {
      value,
      totalValue,
  ...
  const donut = useMemo(() => {
    const TOP = 5;
    const top = partyData.slice(0, TOP);
    const rest = partyData.slice(TOP);
    ...
    <div className="space-y-6 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-indigo-600 uppercase tracking-wider">
            ...
            <span>•</span>
            <span>TCS Receivable Analytics</span>
          ...
          <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">
            TCS Receivable Analytics
          </h1>
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-indigo-600">{period}</span> • TCS receivable from purchases •{" "}
        ...
        {typeof onClose === "function" && (
          <button
            ...
            title="Return to TCS Receivable Report"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Total Bill Value</span>
          <span className="text-sm font-black text-indigo-600">{fmtINR(totals.totalValue)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Amount Paid</span>
          <span className="text-sm font-black text-emerald-600">{fmtINR(totals.paid)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Top TCS Rate</span>
          <span className="text-sm font-black text-violet-600">{totals.topRate ? `${totals.topRate}%` : "-"}</span>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Parties by TCS Receivable</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Highest TCS receivable per party
            </p>

            {partyData.slice(0, 8).length === 0 ? (
              ...
              <div className="h-64 sm:h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={partyData.slice(0, 8)} margin={{ top: 10, right: 10, left: -6, bottom: 0 }}>
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">TCS by Party</h3>
              <span className="text-xs font-bold text-slate-400">{donut.length} parties</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Where receivable TCS concentrates</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Records by TCS Receivable</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Highest TCS receivable bills in this period
            </p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100">
                  ...
                  <th className="py-2.5 px-3 font-bold">Bill No.</th>
                  <th className="py-2.5 px-3 font-bold">Section</th>
                  <th className="py-2.5 px-3 font-bold text-right">Total Value</th>
                  <th className="py-2.5 px-3 font-bold text-right">TCS Receivable</th>
              ...
              <tbody>
                {[...data]
                  ...
                  .map((r, i) => {
                    ...
                    return (
                      <tr key={`${i}-${r.date}-${r.bill}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                        <td className="py-3 pr-3">
                          <span className="w-5 h-5 rounded-md bg-indigo-100 text-indigo-700 font-bold text-[10px] flex items-center justify-center">
                            {i + 1}
                          </span>
                        </td>
                        <td className="py-3 px-3 font-semibold text-slate-600 whitespace-nowrap">{r.date || "-"}</td>
                        <td className="py-3 px-3 font-bold text-slate-800 max-w-[200px] truncate" title={r.name}>
                          {r.name || "-"}
                        </td>
                        <td className="py-3 px-3 font-mono text-[11px] text-slate-500 whitespace-nowrap">{r.bill}</td>
                        <td className="py-3 px-3 font-mono text-[11px] text-slate-500">{r.section}</td>
                        <td className="py-3 px-3 text-right font-semibold text-slate-700 tabular-nums">{fmtINR(r.totalValue)}</td>
