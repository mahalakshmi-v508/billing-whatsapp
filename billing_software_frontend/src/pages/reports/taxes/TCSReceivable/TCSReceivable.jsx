...
import TCSReceivableAnalytics from "./TCSReceivableAnalytics";
...
function printReport(element) {
  ...
  doc.open();
  doc.write(`<html><head><title>TCS Receivable</title><style>body{font-family:Arial,sans-serif;padding:22px;color:#1e1b4b}table{width:100%;border-collapse:collapse;font-size:9px}th,td{border:1px solid #dbe2ec;padding:6px}th{background:#f2f4f7}td.r,th.r{text-align:right}.totals{text-align:right;margin-top:16px;line-height:1.8;font-size:11px}</style></head><body>${element.innerHTML}</body></html>`);
  doc.close();
...
export default function TCSReceivable() {
    ...
    const timer = setTimeout(() => {
      ...
      api
        ...
        .then((res) => {
          ...
          else setError(res.data?.message || "Failed to load TCS Receivable.");
        })
        .catch(() => {
          if (active) setError("Failed to load TCS Receivable.");
        })
        .finally(() => {
  ...
  const totals = useMemo(
    () => ({
      totalValue: filteredRows.reduce((sum, row) => sum + Number(row.total_value || 0), 0),
      tcs: filteredRows.reduce((sum, row) => sum + Number(row.total_tcs_paid || 0), 0),
    }),
    [filteredRows]
  );
  ...
  const analyticsRows = useMemo(() => {
    ...
    return filteredRows.map((row) => ({
      ...
      value: Number(row.total_tcs_paid || 0),
      count: 1,
    }));
  }, [filteredRows]);
  ...
  const exportExcel = () => {
    const data = [
      ...
      [`Company: ${companyName}`],
      [],
      ["#", "Party Name", "Bill No", "Total Value", "Amount Paid", "Total Tax Paid (TCS)", "Date of Collection", "Tax Name", "Tax Section", "Collection Code", "TCS Rate (%)"],
    ...
    XLSX.utils.book_append_sheet(book, sheet, "TCS Receivable");
    const buffer = XLSX.write(book, { bookType: "xlsx", type: "array" });
    saveAs(new Blob([buffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }), "TCS_Receivable.xlsx");
  };
  ...
  const print = () => {
    const body = filteredRows
      .map(
        (row, index) =>
          `<tr><td>${index + 1}</td><td>${row.party_name}</td><td>${row.bill_no}</td><td class="r">${money(row.total_value)}</td><td class="r">${money(row.amount_paid)}</td><td class="r">${money(row.total_tcs_paid)}</td><td>${row.date_of_collection}</td><td>${row.tax_name}</td><td>${row.tax_section}</td><td>${row.collection_code}</td><td class="r">${row.tcs_rate}%</td></tr>`
      )
      .join("");
    const element = document.createElement("div");
    element.innerHTML = `<h2>TCS Receivable</h2><p>From ${iso(fromDate)} To ${iso(toDate)} | ${companyName}</p><table><thead><tr><th>#</th><th>PARTY NAME</th><th>BILL NO</th><th class="r">TOTAL VALUE</th><th class="r">AMOUNT PAID</th><th class="r">TOTAL TAX PAID (TCS)</th><th>DATE OF COLLECTION</th><th>TAX NAME</th><th>TAX SECTION</th><th>COLLECTION CODE</th><th class="r">TCS RATE (%)</th></tr></thead><tbody>${body}</tbody></table><div class="totals">Total Sale With TCS: ${money(totals.totalValue)}<br/>Total TCS: ${money(totals.tcs)}</div>`;
    printReport(element);
  };
  ...
  return (
    <div className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 space-y-3.5 max-w-[1600px] mx-auto text-slate-800 font-sans">
      {viewMode === "analytics" ? (
        <TCSReceivableAnalytics
          rows={filteredRows}
          period={`${iso(fromDate)} → ${iso(toDate)}`}
      ...
      <div className="bg-white rounded-2xl p-5 md:p-6 shadow-xs border border-slate-200/80 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">TCS Receivable</h1>
              <p className="text-xs font-semibold text-slate-400 mt-0.5">
                Tax Collected at Source receivable from suppliers & purchases
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-4 md:p-5 border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-[11px] font-medium text-slate-400 mt-0.5">TCS receivable entries</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 md:p-5 border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            <div className="text-[11px] font-bold tracking-wider uppercase text-slate-500">Total Purchase Value</div>
        ...
        <div className="bg-white rounded-2xl p-4 md:p-5 border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            <div className="text-[11px] font-bold tracking-wider uppercase text-slate-500">TCS Receivable</div>
            <div className="text-xl font-black text-violet-600 mt-0.5">
              {money(totals.tcs)}
            </div>
            <div className="text-[11px] font-medium text-slate-400 mt-0.5">Tax credit claimable</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 md:p-5 border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            <div className="text-[11px] font-bold tracking-wider uppercase text-slate-500">Effective Rate</div>
            <div className="text-xl font-black text-amber-600 mt-0.5">
              {filteredRows.length > 0 && totals.totalValue > 0
                ? ((totals.tcs / totals.totalValue) * 100).toFixed(2)
                : "0.00"}
              %
            </div>
            <div className="text-[11px] font-medium text-slate-400 mt-0.5">TCS to total value %</div>
          </div>
        </div>
      </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan={11} className="px-4 py-16 text-center text-slate-400 font-medium">
                    ...
                    Loading TCS Receivable records...
                  </td>
                </tr>
              ) : error ? (
                <tr>
                ...
                <tr>
                  <td colSpan={11} className="px-4 py-16 text-center text-slate-400 font-medium">
                    No data is available for TCS Receivable.
                    <br />
                ...
                pagedRows.map((row, index) => (
                  <tr key={`${row.bill_no}-${row.date_of_collection}-${index}`} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-3.5 py-3 text-right font-medium text-slate-600">{money(row.amount_paid)}</td>
                    <td className="px-3.5 py-3 text-right font-bold text-indigo-600">{money(row.total_tcs_paid)}</td>
                    <td className="px-3.5 py-3 text-slate-500 whitespace-nowrap">{row.date_of_collection}</td>
            ...
            {!loading && !error && filteredRows.length > 0 && (
              <tfoot className="border-t-2 border-slate-200 bg-slate-50/90 font-bold text-slate-800 text-xs">
                <tr>
                  ...
                  <td className="px-3.5 py-3.5 text-right font-extrabold text-slate-900">{money(totals.totalValue)}</td>
                  <td className="px-3.5 py-3.5"></td>
                  <td className="px-3.5 py-3.5 text-right font-extrabold text-indigo-600">{money(totals.tcs)}</td>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-2xl bg-indigo-50/50 border border-indigo-100 text-xs text-indigo-950 font-semibold">
        <span>
          Total Sale With TCS: <strong className="text-indigo-900 ml-1">{money(totals.totalValue)}</strong>
        </span>
        <span>
          Total TCS: <strong className="text-indigo-900 ml-1">{money(totals.tcs)}</strong>
        </span>
      </div>
