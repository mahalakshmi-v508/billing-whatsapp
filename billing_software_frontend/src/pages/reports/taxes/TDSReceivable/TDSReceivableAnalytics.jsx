...
export default function TDSReceivableAnalytics({ rows = [], period = "", onClose }) {
  const data = useMemo(
    () =>
      (rows || []).map((r) => ({
        name: r.party_name || "General",
        invoice: r.invoice_no || "-",
        date: r.date_of_deduction || "",
        section: r.tax_section || "-",
        rate: Number(r.tds_rate || 0),
        total: Number(r.total_amount || 0),
        taxable: Number(r.taxable_amount || 0),
        value: Number(r.tds_receivable || 0),
      })),
    [rows]
  );
    ...
    <div className="space-y-6 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-indigo-600 uppercase tracking-wider">
            ...
            <span>•</span>
            <span>TDS Receivable Analytics</span>
          ...
          <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">
            TDS Receivable Analytics
          </h1>
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-indigo-600">{period}</span> • TDS receivable claimable per party •{" "}
        ...
        {typeof onClose === "function" && (
          <button
            ...
            title="Return to TDS Receivable Report"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          ...
          <span className="text-sm font-black text-indigo-600">{fmtINR(totals.total)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Taxable Amount</span>
          <span className="text-sm font-black text-emerald-600">{fmtINR(totals.taxable)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Top TDS Rate</span>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Parties by TDS Receivable</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Highest receivable TDS per party
            </p>
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">TDS by Party</h3>
              <span className="text-xs font-bold text-slate-400">{donut.length} parties</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Where receivable TDS concentrates</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Records by TDS Receivable</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Highest receivable TDS invoices in this period
            </p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100">
                  ...
                  <th className="py-2.5 px-3 font-bold text-right">Taxable Amount</th>
                  <th className="py-2.5 px-3 font-bold text-right">TDS Receivable</th>
              ...
              <tbody>
                {[...data]
                  ...
                  .map((r, i) => {
                    ...
                    return (
                      <tr key={`${i}-${r.date}-${r.invoice}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                        ...
                        <td className="py-3 px-3 font-mono text-[11px] text-slate-500 whitespace-nowrap">{r.invoice}</td>
                        <td className="py-3 px-3 font-mono text-[11px] text-slate-500">{r.section}</td>
                        <td className="py-3 px-3 text-right font-semibold text-slate-700 tabular-nums">{fmtINR(r.taxable)}</td>
