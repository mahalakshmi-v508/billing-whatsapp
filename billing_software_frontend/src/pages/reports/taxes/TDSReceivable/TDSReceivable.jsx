...
import TDSReceivableAnalytics from "./TDSReceivableAnalytics";
...
function printReport(element) {
  ...
  doc.open();
  doc.write(`<html><head><title>TDS Receivable</title><style>body{font-family:Arial,sans-serif;padding:22px;color:#1e1b4b}table{width:100%;border-collapse:collapse;font-size:9px}th,td{border:1px solid #dbe2ec;padding:6px}th{background:#f2f4f7}td.r,th.r{text-align:right}.totals{text-align:right;margin-top:16px;line-height:1.8;font-size:11px}</style></head><body>${element.innerHTML}</body></html>`);
  doc.close();
...
export default function TDSReceivable() {
    ...
    const timer = setTimeout(() => {
      ...
      api
        ...
        .then((res) => {
          ...
          else setError(res.data?.message || "Failed to load TDS Receivable.");
        })
        .catch(() => {
          if (active) setError("Failed to load TDS Receivable.");
        })
        .finally(() => {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) =>
      [row.party_name, row.transaction_type, row.invoice_no, row.tax_name, row.tax_section, row.collection_code].some(
  ...
  const totals = useMemo(
    () => ({
      ...
      tdsReceivable: filteredRows.reduce((sum, row) => sum + Number(row.tds_receivable || 0), 0),
    }),
    [filteredRows]
  );
  ...
  const exportExcel = () => {
    const data = [
      ...
      [`Company: ${companyName}`],
      [],
      ["#", "Party Name", "Transaction Type", "Invoice No", "Total Amount", "Taxable Amount", "TDS Receivable", "Date of Deduction", "Tax Name", "Tax Section", "Collection Code", "TDS Rate (%)"],
      ...
      ["", "Total TDS Receivable", totals.tdsReceivable],
    ];
    const sheet = XLSX.utils.aoa_to_sheet(data);
    const book = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(book, sheet, "TDS Receivable");
    const buffer = XLSX.write(book, { bookType: "xlsx", type: "array" });
    saveAs(new Blob([buffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }), "TDS_Receivable.xlsx");
  };
  ...
  const print = () => {
    const body = filteredRows
      .map(
        (row, index) =>
          `<tr><td>${index + 1}</td><td>${row.party_name}</td><td>${row.transaction_type}</td><td>${row.invoice_no}</td><td class="r">${money(row.total_amount)}</td><td class="r">${money(row.taxable_amount)}</td><td class="r">${money(row.tds_receivable)}</td><td>${row.date_of_deduction}</td><td>${row.tax_name}</td><td>${row.tax_section}</td><td>${row.collection_code}</td><td class="r">${row.tds_rate}%</td></tr>`
      )
      .join("");
    const element = document.createElement("div");
    element.innerHTML = `<h2>TDS Receivable</h2><p>From ${iso(fromDate)} To ${iso(toDate)} | ${companyName}</p><table><thead><tr><th>#</th><th>PARTY NAME</th><th>TRANSACTION TYPE</th><th>INVOICE NO</th><th class="r">TOTAL AMOUNT</th><th class="r">TAXABLE AMOUNT</th><th class="r">TDS RECEIVABLE</th><th>DATE OF DEDUCTION</th><th>TAX NAME</th><th>TAX SECTION</th><th>COLLECTION CODE</th><th class="r">TDS RATE (%)</th></tr></thead><tbody>${body}</tbody></table><div class="totals">Total TDS Receivable: ${money(totals.tdsReceivable)}</div>`;
    printReport(element);
  };
      ...
      <div className="bg-white rounded-2xl p-5 md:p-6 shadow-xs border border-slate-200/80 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">TDS Receivable</h1>
              <p className="text-xs font-semibold text-slate-400 mt-0.5">
                Tax Deducted at Source receivable claimable against invoices
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              ...
              placeholder="Search party, invoice, code..."
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-4 md:p-5 border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-[11px] font-medium text-slate-400 mt-0.5">TDS receivable invoices</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 md:p-5 border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-[11px] font-medium text-slate-400 mt-0.5">Total billing value</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 md:p-5 border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            <div className="text-[11px] font-bold tracking-wider uppercase text-slate-500">TDS Receivable</div>
            <div className="text-xl font-black text-violet-600 mt-0.5">
              {money(totals.tdsReceivable)}
            </div>
            <div className="text-[11px] font-medium text-slate-400 mt-0.5">TDS deducted by buyers</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 md:p-5 border border-slate-200/80 shadow-xs flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-[11px] font-medium text-slate-400 mt-0.5">Base eligible amount</div>
          </div>
        </div>
      </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan={12} className="px-4 py-16 text-center text-slate-400 font-medium">
                    ...
                    Loading TDS Receivable records...
                  </td>
                </tr>
              ) : error ? (
                <tr>
                ...
                pagedRows.map((row, index) => (
                  <tr key={`${row.invoice_no}-${row.date_of_deduction}-${index}`} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-3.5 py-3 text-right font-bold text-violet-600">{money(row.tds_receivable)}</td>
            ...
            {!loading && !error && filteredRows.length > 0 && (
              <tfoot className="border-t-2 border-slate-200 bg-slate-50/90 font-bold text-slate-800 text-xs">
                <tr>
                  ...
                  <td className="px-3.5 py-3.5 text-right font-extrabold text-violet-600">{money(totals.tdsReceivable)}</td>
