...
export default function GSTReportAnalytics({ rows = [], period = "", onClose }) {
  const data = useMemo(
    () =>
      (rows || []).map((r) => ({
        name: r.party_name || "General",
        taxIn: Number(r.sale_tax || 0),
        taxOut: Number(r.purchase_expense_tax || 0),
    ...
    <div className="space-y-6 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-indigo-600 uppercase tracking-wider">
            ...
            <span>•</span>
            <span>GST Report Analytics</span>
          ...
          <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">
            GST Report Analytics
          </h1>
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-indigo-600">{period}</span> • GST summary by party •{" "}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Sale + purchase/expense tax combined
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Tax In (Sale)</span>
          <span className="text-sm font-black text-emerald-600">{fmtINR(totals.taxIn)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Tax Out (Purchase/Expense)</span>
          <span className="text-sm font-black text-blue-600">{fmtINR(totals.taxOut)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Net Tax</span>
          <span className="text-sm font-black text-violet-600">{fmtINR(totals.net)}</span>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Parties by Tax</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Highest combined tax contribution per party
            </p>
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">Tax by Party</h3>
              <span className="text-xs font-bold text-slate-400">{donut.length} parties</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Where your tax liability concentrates</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Split of sale tax vs purchase/expense tax per party
            </p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100">
                  ...
                  <th className="py-2.5 px-3 font-bold text-right">Tax In (Sale)</th>
                  <th className="py-2.5 px-3 font-bold text-right">Tax Out (Purchase/Expense)</th>
