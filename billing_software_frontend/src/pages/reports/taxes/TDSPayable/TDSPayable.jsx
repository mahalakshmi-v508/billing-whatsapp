...
import TDSPayableAnalytics from "./TDSPayableAnalytics";
...
export default function TDSPayable() {
  ...
  const analyticsRows = useMemo(() => {
    ...
    return filteredRows.map((row) => ({
      ...
      value: Number(row.tds_payable || 0),
      count: 1,
    }));
  }, [filteredRows]);
      ...
      <div className="bg-white rounded-2xl p-5 md:p-6 shadow-xs border border-slate-200/80 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">TDS Payable</h1>
