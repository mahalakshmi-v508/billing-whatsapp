...
export default function FormNo27EQAnalytics({ rows = [], period = "", onClose }) {
  const data = useMemo(
    () =>
      (rows || []).map((r) => ({
        name: r.party_name || r.tax_name || "General",
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      avg: data.length ? tcs / data.length : 0,
    };
  }, [data]);
    ...
    <div className="space-y-6 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          ...
          <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">
            Form No. 27EQ Analytics
          </h1>
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-indigo-600">{period}</span> • TCS collected on sales •{" "}
        ...
        {typeof onClose === "function" && (
          <button
            ...
            title="Return to Form No. 27EQ Report"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Tax collected at source on sales
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Collection transactions logged
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Unique parties contributing TCS
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Total Sale Value</span>
          <span className="text-sm font-black text-indigo-600">{fmtINR(totals.totalValue)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Amount Received</span>
          <span className="text-sm font-black text-emerald-600">{fmtINR(totals.amountReceived)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Total Tax</span>
          <span className="text-sm font-black text-violet-600">{fmtINR(totals.totalTax)}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Parties by TCS</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Highest TCS collected per party
            </p>
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            ...
            <p className="text-xs text-slate-500 mb-4">Where your TCS collections concentrate</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Transactions by TCS</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Highest TCS transactions in this period
            </p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100">
                  ...
                  <th className="py-2.5 px-3 font-bold text-right">TCS</th>
