...
const GROUP_COLORS = ["#6366f1", "#0891b2", "#10b981", "#f59e0b", "#7c3aed", "#f43f5e", "#eab308"];
...
function CustomCustomerTooltip({ active, payload }) {
  if (!active || !payload?.length) return null;
  ...
  return (
    <div className="bg-slate-900/95 backdrop-blur-md text-white rounded-xl px-3.5 py-2.5 shadow-2xl border border-slate-700/50 text-xs">
      <div className="font-bold text-slate-200 flex items-center gap-2">
        ...
        {item.payload.isOther ? "Other Customers" : <Capitalized text={item.name} />}
      </div>
      <div className="text-slate-400 mt-1">
        Invoiced: <span className="text-white font-semibold">{fmtINR(item.value)}</span>
      </div>
      <div className="text-slate-400">
        Invoices: <span className="text-white font-semibold">{item.payload.count}</span> ({item.payload.percent}%)
      </div>
    </div>
  );
}
...
export default function SaleAnalytics({
  ...
  const customerBreakdown = useMemo(() => {
    const map = new Map();
    rows.forEach((r) => {
      const name = (r.group || "Cash Sale").trim();
  ...
  const customerDonut = useMemo(() => {
    const TOP = 5;
    const top = customerBreakdown.slice(0, TOP);
    const rest = customerBreakdown.slice(TOP);
  ...
  const customerMax = Math.max(...customerBreakdown.map((c) => c.value), 1);
    ...
    <div className="space-y-6 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          ...
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-indigo-600">{tooltipPeriod}</span> •{" "}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Outstanding receivable from customers
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            {paymentMethods.map((m) => (
              <div key={m.name} className="bg-slate-50/80 rounded-xl p-3 border border-slate-200/70">
                ...
                <div className="font-extrabold text-indigo-600 text-sm mt-1">{fmtINR(m.value)}</div>
                <div className="text-[10px] text-slate-400 mt-1">
                  {m.count} {m.count === 1 ? "invoice" : "invoices"}
                </div>
            ...
            <div className="h-64 sm:h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={daily.list} margin={{ top: 10, right: 10, left: -6, bottom: 0 }}>
                  ...
                  {metricView === "value" ? (
                    <Bar dataKey="value" name="Invoice Value" fill="#6366f1" radius={[6, 6, 0, 0]} maxBarSize={34} />
                  ) : (
                    <Bar dataKey="count" name="Invoices" fill="#8b5cf6" radius={[6, 6, 0, 0]} maxBarSize={34} />
                  )}
                </BarChart>
              </ResponsiveContainer>
            </div>
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">By Customer</h3>
              <span className="text-xs font-bold text-slate-400">{customerBreakdown.length} customers</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Invoice value shared by each customer</p>
            ...
            <div className="space-y-2.5 mt-4">
              {customerDonut.map((item) => (
                <div key={item.name} className="space-y-1">
                  <div className="flex items-center justify-between text-[11px]">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: item.color }} />
                      <span className="font-semibold text-slate-700 truncate">
                        {item.isOther ? "Other Customers" : <Capitalized text={item.name} />}
                      </span>
                    </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Customers by Invoice Value</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Collected vs pending breakdown for every customer in the period
            </p>
          </div>
          <span className="text-xs font-semibold text-slate-500">
            {customerBreakdown.length} customer{customerBreakdown.length === 1 ? "" : "s"}
          </span>
        </div>

        {customerBreakdown.length === 0 ? (
          <div className="text-center py-6 text-slate-400 text-xs">No customer data available.</div>
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100">
                  ...
                  <th className="py-2.5 px-3 font-bold text-right">Invoices</th>
                  <th className="py-2.5 px-3 font-bold text-right">Value</th>
                  <th className="py-2.5 px-3 font-bold text-right">Collected</th>
                  <th className="py-2.5 px-3 font-bold text-right">Pending</th>
                  <th className="py-2.5 pl-3 font-bold">Collected vs Pending</th>
                </tr>
              </thead>
              <tbody>
                {customerBreakdown.slice(0, 10).map((c, i) => {
                  const value = Math.max(c.value, 1);
                  const paidPct = (c.paid / value) * 100;
                  const pendingPct = (c.balance / value) * 100;
                  const share = (c.value / customerMax) * 100;
                  return (
                    <tr key={`${c.name}-${i}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                      <td className="py-3 pr-3">
                        <span className="w-5 h-5 rounded-md bg-indigo-100 text-indigo-700 font-bold text-[10px] flex items-center justify-center">
                          {i + 1}
                        </span>
                      </td>
                      <td className="py-3 px-3 font-bold text-slate-800 max-w-[220px] truncate" title={c.name}>
                        {c.name}
                      </td>
                      <td className="py-3 px-3 text-right text-slate-500 font-semibold">{c.count}</td>
                      <td className="py-3 px-3 text-right font-extrabold text-slate-900">{fmtINR(c.value)}</td>
                      <td className="py-3 px-3 text-right font-bold text-emerald-600">{fmtINR(c.paid)}</td>
                      <td className="py-3 px-3 text-right font-bold text-rose-600">{fmtINR(c.balance)}</td>
