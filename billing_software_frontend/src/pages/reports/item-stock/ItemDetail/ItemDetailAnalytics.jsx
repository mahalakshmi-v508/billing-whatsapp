  ...
  return (
    <div className="bg-slate-900/95 backdrop-blur-md text-white rounded-xl px-3.5 py-2.5 shadow-2xl border border-slate-700/50 text-xs">
      <div className="font-bold text-slate-200 flex items-center gap-2">
        ...
        {item.payload.isOther ? "Other Days" : <Capitalized text={item.name} />}
      </div>
      <div className="text-slate-400 mt-1">
        Closing Qty: <span className="text-white font-semibold">{fmtQty(item.value)}</span>
      </div>
...
export default function ItemDetailAnalytics({ rows = [], period = "", onClose }) {
  const data = useMemo(
    () =>
      (rows || [])
        .map((r) => ({
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      lastVal: last ? last.value : 0,
      lastDate: last ? last.name : "-",
    ...
    <div className="space-y-6 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-indigo-600 uppercase tracking-wider">
            ...
            <span>•</span>
            <span>Item Detail Analytics</span>
          ...
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-indigo-600">{period}</span> • Closing quantity trend per day •{" "}
            {totals.count} day{totals.count === 1 ? "" : "s"}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Daily closing quantity records
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div>
            ...
            <div className="text-xl sm:text-2xl font-black text-amber-600 mt-2 tracking-tight truncate" title={chartData[0]?.name}>
              {chartData[0]?.value ? fmtQty(chartData[0].value) : "-"}
            </div>
          </div>
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Highest closing quantity
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Avg / Day</span>
          <span className="text-sm font-black text-emerald-600">{fmtQty(totals.avg)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Peak Day</span>
          <span className="text-sm font-black text-blue-600">{chartData[0]?.value ? fmtQty(chartData[0].value) : "-"}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Total Days</span>
          <span className="text-sm font-black text-violet-600">{totals.count}</span>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Closing Quantity Trend</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Daily closing quantity (most recent days)
            </p>

            {chartData.slice(0, 21).length === 0 ? (
              ...
              <div className="h-64 sm:h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData.slice(0, 21)} margin={{ top: 10, right: 10, left: -6, bottom: 0 }}>
                    ...
                    <Bar dataKey="value" name="Closing Qty" fill="#6366f1" radius={[6, 6, 0, 0]} maxBarSize={36} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">Qty by Day</h3>
              <span className="text-xs font-bold text-slate-400">{donut.length} days</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Where closing quantity concentrates by day</p>
                  ...
                  <div className="flex items-center justify-between text-[11px]">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="font-semibold text-slate-700 truncate">
                        {item.isOther ? "Other Days" : <Capitalized text={item.name} />}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="font-bold text-slate-900">{fmtQty(item.value)}</span>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Days by Closing Qty</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Per-day closing quantity
            </p>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100">
                  ...
                  <th className="py-2.5 px-3 font-bold text-right">Closing Qty</th>
                  ...
                  .map((r, i) => {
                    ...
                    return (
                      <tr key={`${i}-${r.name}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                        ...
                        <td className="py-3 px-3 text-right font-extrabold text-indigo-600 tabular-nums">{fmtQty(r.value)}</td>
