...
function invoicesOf(g) {
  return g?.invoicesNo instanceof Set ? g.invoicesNo.size : Number(g?.invoicesNo || 0) || 1;
}
function labelOf(g) {
  return `${g?.sac || "NA"}${g?.invoiceType ? ` · ${g.invoiceType}` : ""}`;
}

function CustomBarTooltip({ active, payload, label }) {
  if (!active || !payload?.length) return null;
  return (
    <div className="bg-slate-900/95 backdrop-blur-md text-white rounded-xl px-4 py-3 shadow-2xl border border-slate-700/50 text-xs max-w-[260px]">
  ...
  return (
    <div className="bg-slate-900/95 backdrop-blur-md text-white rounded-xl px-3.5 py-2.5 shadow-2xl border border-slate-700/50 text-xs">
      <div className="font-bold text-slate-200 flex items-center gap-2">
        ...
        {item.payload.isOther ? "Other SACs" : <Capitalized text={item.name} />}
      </div>
...
export default function SACReportAnalytics({ rows = [], period = "", onClose }) {
  const totals = useMemo(() => {
    ...
    rows.forEach((g) => {
      ...
      invoices += invoicesOf(g);
    });
    return { base, tax, total, invoices, count: rows.length, rate: base ? (tax / base) * 100 : 0 };
  }, [rows]);

  const topRows = useMemo(
    () =>
      [...rows]
        .map((g) => ({ ...g, label: labelOf(g) }))
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-indigo-600 uppercase tracking-wider">
            <span>GST</span>
            <span>•</span>
            <span>SAC Report Analytics</span>
          ...
          <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">
            SAC Report Analytics
          </h1>
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-indigo-600">{period}</span> • SAC-wise Summary •{" "}
            {totals.count} SAC code{totals.count === 1 ? "" : "s"}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            {totals.invoices} invoice{totals.invoices === 1 ? "" : "s"} covered
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Distinct service accounting codes
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top SAC Codes by Base</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Taxable base value for the top service codes
            </p>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={topRows} margin={{ top: 10, right: 10, left: -6, bottom: 0 }}>
                    ...
                    <XAxis
                      ...
                      tickFormatter={(v) => (v ? v.slice(0, 12) : "NA")}
                      axisLine={false}
                      tickLine={false}
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">Base by SAC</h3>
              <span className="text-xs font-bold text-slate-400">{donut.length} codes</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Service value share per SAC code</p>
                  ...
                  <div className="flex items-center justify-between text-[11px]">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="font-semibold text-slate-700 truncate">
                        {item.isOther ? "Other SACs" : <Capitalized text={item.name} />}
                      </span>
                    </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>SAC-wise Summary</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Invoices, base, tax and total value per SAC code
            </p>
          </div>
          <span className="text-xs font-semibold text-slate-500">{totals.count} SAC codes</span>
        </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] uppercase tracking-wider text-slate-400 border-b border-slate-100">
                  ...
                  <th className="py-2.5 px-3 font-bold">SAC</th>
                  <th className="py-2.5 px-3 font-bold">Invoice Type</th>
              ...
              <tbody>
                {[...rows]
                  ...
                  .map((g, i) => {
                    ...
                    return (
                      <tr key={`${g.sac}-${g.invoiceType}-${i}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                        ...
                        <td className="py-3 px-3 font-bold text-slate-800">
                          <span className="inline-flex px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 text-[10px] font-bold">{g.sac || "NA"}</span>
                        </td>
                        <td className="py-3 px-3 font-semibold text-slate-600 max-w-[160px] truncate" title={g.invoiceType}>
                          <Capitalized text={g.invoiceType || "-"} />
                        </td>
                        <td className="py-3 px-3 text-right font-semibold text-slate-500">{invoicesOf(g)}</td>
