...
export default function GstR2Analytics({ rows = [], period = "", onClose }) {
  const totals = useMemo(() => {
    ...
    rows.forEach((r) => {
      ...
      if (r.party_name) parties.add(String(r.party_name));
    });
    return { value, count, parties: parties.size, avg: count ? value / count : 0 };
  }, [rows]);

  const topParties = useMemo(() => {
    const map = new Map();
    rows.forEach((r) => {
      const key = r.party_name || "General";
  ...
  const donut = useMemo(() => {
    ...
    const data = top.map((p, i) => ({ name: p.name, value: p.value, color: COLORS[i % COLORS.length] }));
    if (rest.length) {
      data.push({ name: "Others", value: rest.reduce((a, p) => a + p.value, 0), color: "#cbd5e1", isOther: true });
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          ...
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-indigo-600">{period}</span> • Inward Supplies •{" "}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Taxable value of inward supplies
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Parties by Value</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Highest inward supply value per supplier
            </p>
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">Value by Party</h3>
              <span className="text-xs font-bold text-slate-400">{donut.length} parties</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Where your inward supplies concentrate</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Documents by Value</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Highest value invoice / debit note records in this period
            </p>
          </div>
              ...
              <tbody>
                {[...rows]
                  ...
                  .map((r, i) => {
                    ...
                    return (
                      <tr key={`${i}-${r.date}-${r.party_name}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                        ...
                        <td className="py-3 px-3 font-bold text-slate-800 max-w-[240px] truncate" title={r.party_name}>
                          {r.party_name || "-"}
                        </td>
