...
export default function PaymentOutAnalytics({ rows = [], period = "", onClose }) {
  const data = useMemo(
    () =>
      rows.map((p, i) => ({
        date: p.payment_date || "",
  ...
  const totals = useMemo(() => {
    ...
    data.forEach((r) => {
      paid += r.paid;
      billed += r.billed;
      if (r.party) suppliers.add(r.party);
      if (r.method) methods.add(r.method);
    });
    return {
      paid,
      billed,
      paidPct: billed ? (paid / billed) * 100 : 0,
      count: data.length,
      suppliers: suppliers.size,
      methods: methods.size,
      avg: data.length ? paid / data.length : 0,
    };
  }, [data]);
  ...
  const methodData = useMemo(() => {
    const map = new Map();
    data.forEach((r) => {
      map.set(r.method, (map.get(r.method) || 0) + r.paid);
    });
    return [...map.entries()]
  ...
  const maxRow = Math.max(...data.map((r) => r.paid), 1);
    ...
    <div className="space-y-6 transition-all">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-purple-600 uppercase tracking-wider">
            <span>Purchase</span>
            <span>•</span>
            <span>Payment-Out Analytics</span>
          ...
          <h1 className="text-xl md:text-2xl font-bold text-slate-900 mt-1 tracking-tight">
            Payment-Out Analytics
          </h1>
          <p className="text-xs md:text-sm text-slate-500 mt-0.5">
            <span className="font-semibold text-purple-600">{period}</span> • Supplier outward payments •{" "}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div>
            ...
            <div className="text-2xl sm:text-3xl font-black text-purple-600 mt-2 tracking-tight">
              {fmtINR(totals.paid)}
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-100">
            Unique suppliers in this period
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Total Bill Amount</span>
          <span className="text-sm font-black text-slate-800">{fmtINR(totals.billed)}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Payment Methods</span>
          <span className="text-sm font-black text-indigo-600">{totals.methods}</span>
        </div>
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Paid vs Billed</span>
          <span className="text-sm font-black text-emerald-600">{totals.paidPct.toFixed(1)}%</span>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5 mb-4">
              Highest total paid by payment method
            </p>
              ...
              <div className="h-64 sm:h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={methodData.slice(0, 8)} margin={{ top: 10, right: 10, left: -6, bottom: 0 }}>
                    ...
                    <Bar dataKey="value" name="Paid" fill="#7c3aed" radius={[6, 6, 0, 0]} maxBarSize={36} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 shadow-xs flex flex-col">
          <div>
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-sm font-bold text-slate-900 tracking-tight">Paid by Method</h3>
              <span className="text-xs font-bold text-slate-400">{totals.methods} methods</span>
            </div>
            <p className="text-xs text-slate-500 mb-4">Where your payments go out</p>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>Top Payment-Out Vouchers by Paid</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Highest outward payment records in this period
            </p>
          </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {[...data]
                  ...
                  .slice(0, 20)
                  .map((r, i) => {
                    const share = (Number(r.paid) / maxRow) * 100;
                    return (
                      <tr key={`${i}-${r.date}-${r.receipt}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition text-xs">
                        <td className="py-3 pr-3">
                          <span className="w-5 h-5 rounded-md bg-purple-100 text-purple-700 font-bold text-[10px] flex items-center justify-center">
                            {i + 1}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-3">
                          <span className="inline-flex px-2 py-0.5 rounded-full bg-purple-50 text-purple-600 font-bold text-[10px]">
                            <Capitalized text={r.method} />
                          </span>
                        </td>
                        <td className="py-3 px-3 text-right font-extrabold text-purple-600">{fmtINR(r.paid)}</td>
                        <td className="py-3 pl-3 min-w-[120px]">
                          <div className="flex items-center gap-2">
                            <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden shrink-0">
                              <div className="bg-purple-500 h-1.5 rounded-full" style={{ width: `${Math.min(100, share)}%` }} />
                            </div>
