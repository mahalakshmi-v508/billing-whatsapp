    ...
    (async () => {
      ...
      try {
        ...
        if (res.data?.status) {
          ...
          const fullLogoUrl = getLogoUrl(c.logo) || null;
          setForm({
            name: c.company_name || "",
            code: c.company_code || "",
        ...
        toast("Server error while loading company data", "error");
      } finally {
        setFetching(false);
      }
    })();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isSuccess) {
        ...
        toast(res.data?.message || res.data?.msg || "Failed to update company", "error");
      }
    } catch (err) {
    ...
    <div className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200/90 overflow-hidden flex flex-col font-['Plus_Jakarta_Sans',sans-serif] max-h-[90vh] transition-all animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Update business entity profile, GST tax details &amp; branding
            </p>
          </div>
        </div>
      ...
      {fetching ? (
        <div className="p-12 flex flex-col items-center justify-center gap-3">
          ...
          <span className="text-xs font-bold text-slate-600">Loading company profile...</span>
        </div>
      ) : (
        /* ── FORM BODY ── */
              ...
              <div className="flex items-center gap-4 p-3 bg-slate-50 rounded-2xl border border-slate-200">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-slate-400">
                    {form.logo ? `${Math.round(form.logo.size / 1024)} KB` : "Current logo"}
                  </p>
                </div>
